...
# pipx package parameters for Ansible
INCLUDEANSIBLE="true"
ANSIBLEVERSION=${VERSION:-"latest"}
# pipx injection parameters for Ansible env   
    INCLUDEANSIBLECOMMUNITY=${INCLUDEANSIBLECOMMUNITY:-"true"}
    ANSIBLECOMMUNITYVERSION=${ANSIBLECOMMUNITYVERSION:-"latest"}
...
if [ "$INCLUDEANSIBLE" = "true" ]; then
    if [ "$ANSIBLEVERSION" =  "latest" ]; then
        util_command="ansible-core"
    else
        util_command="ansible-core==$ANSIBLEVERSION"
    fi
    "${PIPX_COMMAND}" install --system-site-packages --force --pip-args '--no-cache-dir --force-reinstall' ${util_command}
    if [ "$INCLUDEANSIBLECOMMUNITY" =  "true" ]; then
        if [ "$ANSIBLECOMMUNITYVERSION" =  "latest" ]; then
            util_command="ansible"
        else
            util_command="ansible==$ANSIBLECOMMUNITYVERSION"
        fi
    pipx inject ansible-core ${util_command}
    fi
