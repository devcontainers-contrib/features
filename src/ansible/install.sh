...
# pipx version for ansible-core
ANSIBLE=${VERSION:-"latest"}
# injection versions for ansible-core pipx env
...
if [ "$ANSIBLE" != "none" ]; then
	if [ "$ANSIBLE" = "latest" ]; then
		pipx_installations+=("ansible-core")
	else
		pipx_installations+=("ansible-core==$ANSIBLE")
	fi
	if [ "$ANSIBLE_COMMUNITY" != "none" ]; then
		if [ "$ANSIBLE_COMMUNITY" = "latest" ]; then
			pipx_installations+=("ansible")
		else
			pipx_installations+=("ansible==$ANSIBLE_COMMUNITY")
		fi
	fi
