...
# pipx package parameters for qrcode
INCLUDEQRCODE="true"
QRCODEVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEQRCODE" = "true" ]; then
    if [ "$QRCODEVERSION" =  "latest" ]; then
        util_command="qrcode[pil]"
    else
        util_command="qrcode[pil]==$QRCODEVERSION"
    fi
