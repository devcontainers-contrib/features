...
$nanolayer_location \
    ...
    --option repo='actions/runner' --option binaryName='run.sh' --option binLocation='$_REMOTE_USER_HOME/.local/bin' --option version="$VERSION" --option libLocation='$_REMOTE_USER_HOME' --option libName='actions-runner' --option assetRegex='^(?!.*(noexternals))(?!.*(trimmedpackages))(.*-noruntime.*)'
