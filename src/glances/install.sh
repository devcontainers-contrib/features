...
# pipx version for glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]
GLANCES=${VERSION:-"latest"}
...
if [ "$GLANCES" != "none" ]; then
	if [ "$GLANCES" = "latest" ]; then
		pipx_installations+=("glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]")
	else
		pipx_installations+=("glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]==$GLANCES")
	fi
fi
