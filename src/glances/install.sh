...
if [ "${UTIL_VERSION}" == "latest" ]; then
	util_command="glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]"
else
	util_command="glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi]==$UTIL_VERSION"
fi

echo "Installing glances[action,browser,cloud,cpuinfo,docker,export,folders,gpu,graph,ip,raid,snmp,web,wifi] ..."
