...
# pipx package parameters for datasette
INCLUDEDATASETTE="true"
DATASETTEVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEDATASETTE" = "true" ]; then
    if [ "$DATASETTEVERSION" =  "latest" ]; then
        util_command="datasette"
    else
        util_command="datasette==$DATASETTEVERSION"
    fi
