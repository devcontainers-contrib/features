...
# pipx package parameters for Meltano
INCLUDEMELTANO="true"
MELTANOVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEMELTANO" = "true" ]; then
    if [ "$MELTANOVERSION" =  "latest" ]; then
        util_command="meltano"
    else
        util_command="meltano==$MELTANOVERSION"
    fi
