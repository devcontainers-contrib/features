...
# pipx package parameters for Localstack
INCLUDELOCALSTACK="true"
LOCALSTACKVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDELOCALSTACK" = "true" ]; then
    if [ "$LOCALSTACKVERSION" =  "latest" ]; then
        util_command="localstack[full]"
    else
        util_command="localstack[full]==$LOCALSTACKVERSION"
    fi
