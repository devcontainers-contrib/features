...
# pipx package parameters for youtube-dl
INCLUDEYOUTUBEDL="true"
YOUTUBEDLVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEYOUTUBEDL" = "true" ]; then
    if [ "$YOUTUBEDLVERSION" =  "latest" ]; then
        util_command="youtube-dl"
    else
        util_command="youtube-dl==$YOUTUBEDLVERSION"
    fi
