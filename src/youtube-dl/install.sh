...
# pipx version for youtube-dl
YOUTUBE_DL=${VERSION:-"latest"}
...
pipx_installations=()
if [ "$YOUTUBE_DL" != "none" ]; then
	if [ "$YOUTUBE_DL" = "latest" ]; then
		pipx_installations+=("youtube-dl")
	else
		pipx_installations+=("youtube-dl==$YOUTUBE_DL")
	fi
fi
