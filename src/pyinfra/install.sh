...
# pipx package parameters for pyinfra
INCLUDEPYINFRA="true"
PYINFRAVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEPYINFRA" = "true" ]; then
    if [ "$PYINFRAVERSION" =  "latest" ]; then
        util_command="pyinfra"
    else
        util_command="pyinfra==$PYINFRAVERSION"
    fi
