...
# npm version for @quasar/cli
QUASAR_CLI=${VERSION:-"latest"}
...
if [ "$QUASAR_CLI" != "none" ]; then
	if [ "$QUASAR_CLI" = "latest" ]; then
		util_command="@quasar/cli"
	else
		util_command="@quasar/cli@$QUASAR_CLI"
	fi
