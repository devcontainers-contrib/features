...
# pipx package parameters for mitmproxy
INCLUDEMITMPROXY="true"
MITMPROXYVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEMITMPROXY" = "true" ]; then
    if [ "$MITMPROXYVERSION" =  "latest" ]; then
        util_command="mitmproxy"
    else
        util_command="mitmproxy==$MITMPROXYVERSION"
    fi
