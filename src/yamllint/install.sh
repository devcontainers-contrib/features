...
# pipx version for yamllint
YAMLLINT=${VERSION:-"latest"}
...
pipx_installations=()
if [ "$YAMLLINT" != "none" ]; then
	if [ "$YAMLLINT" = "latest" ]; then
		pipx_installations+=("yamllint")
	else
		pipx_installations+=("yamllint==$YAMLLINT")
	fi
fi
