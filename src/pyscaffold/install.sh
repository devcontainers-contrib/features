...
# pipx version for pyscaffold[all]
PYSCAFFOLD=${VERSION:-"latest"}
...
pipx_installations=()
if [ "$PYSCAFFOLD" != "none" ]; then
	if [ "$PYSCAFFOLD" = "latest" ]; then
		pipx_installations+=("pyscaffold[all]")
	else
		pipx_installations+=("pyscaffold[all]==$PYSCAFFOLD")
	fi
fi
