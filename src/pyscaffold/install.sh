...
# pipx package parameters for PyScaffold
INCLUDEPYSCAFFOLD="true"
PYSCAFFOLDVERSION=${VERSION:-"latest"}
...
if [ "$INCLUDEPYSCAFFOLD" = "true" ]; then
    if [ "$PYSCAFFOLDVERSION" =  "latest" ]; then
        util_command="pyscaffold[all]"
    else
        util_command="pyscaffold[all]==$PYSCAFFOLDVERSION"
    fi
